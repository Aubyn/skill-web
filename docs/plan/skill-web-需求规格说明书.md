...
| R4 | 同步采用硬链接 | 逐文件硬链接（`os.Link`），等同于目录复制 |
| R5 | 覆盖前自动备份 | 备份到 `~/.skill-web/backups/<timestamp>/`；空目标不备份；每目标保留最多 10 个版本 |
...
| F11 展开查看目标目录 | 点击目标目录卡片 | 目录存在 | 调用 API 读取该目录下的子目录，展示技能列表 |
| F12 执行同步 | 选来源（技能/组）+ 选目标 → 确认 → 执行 | 来源+目标有效 | 检查目录存在性 → 确认 → 备份→清空→硬链接部署到位，Toast 提示结果 |
...
| **AC-3** | 选来源（技能/组）+ 目标目录 → 确认同步 → 备份目标 → 清空 → 创建硬链接 → 显示逐技能结果 |
| **AC-3a** | 目标目录不存在时，用户确认后创建 |
| **AC-3b** | 部分硬链接失败时报告失败项，不回滚已成功操作 |
...
| **同步** | 从技能库选来源、从技能组选来源、目标为空、目标有旧内容、目标不存在、**部分硬链接失败**、**路径含 `~`** |
