...
```mermaid
flowchart TD
    A[打开技能组页面] --> B[点击「新建技能组」]
    B --> C[输入技能组名称和描述]
    C --> D[确认创建]
    D --> E[写入 SQLite skill_groups 表]
    E --> F[在组详情页点击「追加技能」]
    F --> G[弹出多选对话框: 搜索 → 勾选技能 → 确认追加]
    G --> H[写入 group_skills 表，左侧列表计数更新]
    H --> I[切换到同步页面]
    I --> J[选择来源方式: 从技能组]
    J --> K[选择技能组]
    K --> L[系统解析组内技能列表]
...
| F4 查看技能详情 | 点击技能卡片 | 技能存在于库 | 展示文件结构+元信息 |
...
| F12 执行同步 | 选来源（技能/组）+ 选目标 → 确认 → 执行 | 来源+目标有效 | 备份→清空→软链部署到位，展示逐技能结果 |

### 6.2 交互设计规格

| 页面 | 加载态 | 空态 | 错误态 | 不可操作态 | 操作后置状态 |
|------|--------|------|--------|------------|-------------|
| **技能库列表** | 骨架屏 | "还没有技能，点击导入开始" | 加载失败提示重试 | — | 导入后列表刷新，弹窗关闭 |
| **技能详情** | 加载中 | — | 技能不存在提示 | — | — |
| **技能组列表** | 骨架屏 | "还没有技能组，新建一个" | 加载失败提示重试 | — | 建组/追加/移除后左侧计数更新 |
| **技能组详情** | 加载中 | 组内无技能提示 | 组不存在提示 | — | 追加后技能列表刷新，弹窗关闭；移除后列表刷新 |
| **技能组追加对话框** | 加载中 | "所有技能已在组中" | — | 未勾选时确认按钮置灰 | 追加后弹窗关闭，技能列表刷新 |
| **目标目录列表** | 骨架屏 | "还没有目标目录，添加一个" | 目录不可读时展示具体错误 | — | 展开时动态加载技能 |
| **同步页面** | 同步中按钮旋转 | "请选择来源/目标" | 同步失败逐项报告 | 无可用来源或目标时按钮置灰 | 同步完成后展示逐技能标签结果 |
...
| **AC-4a** | 技能组为空时同步按钮置灰，提示"该技能组为空，无法同步" |
...
│   └── src/
│       ├── App.tsx          # 路由
│       ├── api/client.ts    # 完整 API 类型客户端
│       ├── components/
│       │   ├── AppShell.tsx  # 侧边栏 + 主内容布局
│       │   └── Sidebar.tsx  # 4 项导航
│       └── pages/
│           ├── SkillLibrary.tsx  # 技能库 + 搜索 + 导入弹窗
│           ├── SkillGroups.tsx   # 分组 + 多选追加对话框
│           ├── SyncPage.tsx      # 双来源同步 + 逐技能结果
│           └── TargetsPage.tsx   # 目标目录 + 展开查看技能
├── skill-web                # 编译后二进制
